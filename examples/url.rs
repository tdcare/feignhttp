...
async fn languages(#[path] owner: &str, #[path] repo: &str) -> feignhttp_rs::Result<String> {}

// Dynamic url.
#[get(url = "{url}", path = "/repos/dxx/feignhttp/languages")]
async fn languages2(#[path] url: &str) -> feignhttp_rs::Result<String> {}

// Lazy loading url.
static URL: Lazy<String> =
    Lazy::new(|| std::env::var("GITHUB_URL").unwrap_or("https://api.github.com".to_string()));

#[get(url = URL, path = "/repos/{owner}/{repo}/languages")]
async fn languages3(#[path] owner: &str, #[path] repo: &str) -> feignhttp_rs::Result<String> {}
